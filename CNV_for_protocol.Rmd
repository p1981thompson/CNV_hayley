...
#generate covariate values (assumption that no correlation between predictors)
 ...
 Group <- rbinom(n,1,prob=184/314) #simulate binary variable (0=comparison, 1=SCT)
...
```{r power} 
 ...
 for(i in 1:100)
 {
 s.data<-data[sample(1:100000,myn,replace=F),]
 ...
 #method 1
# power.int<-length(which(pvec[,1]<=0.05))
# power.sum_pLi<-length(which(pvec[,2]<=0.05))
# power.Group<-length(which(pvec[,3]<=0.05))
# power.sum_pli.Group<-length(which(pvec[,4]<=0.05))
 
# powers<-c(power.int,power.Group,power.sum_pLi,power.sum_pli.Group)
...
#method 2 
 ...
 data.pow<-as.data.frame(t(pows))
 colnames(data.pow)<-c("Intercept","sum_pLi","Group","sum_pLi:Group")
