...
Statistical power is assessed using the R package 'simglm' (LeBeau, 2017) and makes some assumptions about the size of regression parameters for the simulation to be accurate. The parameters should be interpretted on with respect to the log(phenotype) dependent measure. For example, a single point change in the sum_pLi variable would equate to a 1.34 change in the phenotype ($\exp^{0.3}$).

```{r pois_pwr, echo=TRUE}

#---------------------------------------------------------------------------#

# single level Poisson (glm) 


fixed <- ~ 1 + sum_pLi + Group.o + sum_pLi:Group.o

fixed_param <- c(-1.5, 0.3, 0.1, 0.1)
...
#---------------------------------------------------------------------------#
knitr::kable(power_out)

```

The table shows the relative power for each parameter estimate at a nominal N=200, participants. Note this is only correct based on the assumptions made.
...
LeBeau, B. (2017). simglm: Simulate Models Based on the Generalized Linear Model. R package
version 0.6.0. https://CRAN.R-project.org/package=simglm
