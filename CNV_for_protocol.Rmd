...
![Figure 2. *N children with each type of trisomy, divided according to whether diagnosis was made when investigating neurodevelopmental disorder.*](fig2_2018_02_06_fromWOR.png)
...
###Test battery
The test battery is detailed by Newbury et al (2018) and specific measures are described more fully below. It was designed to provide a quantitative estimate of language, literacy and communication ability in children aged 5 to 16 years. In addition, parents completed a telephone interview, and were invited to complete two questionnaires and an online diagnostic interview.
...
Genotyping data also includes 215 samples from first degree relatives of participants. This makes it possible to establish which parent the CNV was inherited from, or if it was de novo, which is particularly useful if a potentially pathogenic CNV is discovered. 
*Hayley to add more information here*

All available data from genotyping will undergo CNV analysis in the current study. [*Hayley: does that include the parents?*]
...
In line with our ethics committee approval, we will describe results of this analysis in a newsletter to participant families, and encourage those who wish to do so to ask for information specific to their child. For cases who have a pathogenic or likely pathogenic CNV, and whose parents request feedback, we will liaise with their local clinical geneticist, so that the finding can be discussed with the family. 
...
```
The remaining scripts are developed to handle these questions:
...
Statistical power is assessed using the R package 'simglm' (LeBeau, 2017), which requires some assumptions about the size of regression parameters for the simulation to be accurate. The parameters should be interpreted with respect to the log(phenotype) dependent measure. For example, a single point change in the sum_pLi variable would equate to a 1.34 change in the phenotype ($\exp^{0.3}$).

```{r pois_pwr, echo=TRUE}
#---------------------------------------------------------------------------#
# single level Poisson (glm) 
#---------------------------------------------------------------------------#
poissonpower <- 0 #to skip this step when running on a Mac: simglm throws errors
#when run on a Mac.
myn <- 130
if(poissonpower==1){
...
The table shows the relative power for each parameter estimate at a nominal N = `r myn`, participants. Note this is only correct based on the assumptions made.
